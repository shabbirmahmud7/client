    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
       <div className='flex item-center gap-3'>
        <Sparkles className='w-6 text-[#edff4a]' />
     ...
     <p className='mt-6 text-sm font-medium'>Describe Object to Remove</p>
      <textarea onChange={(e)=>setObject(e.target.value)} value={object} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='e.g, watch or spoon , only single, object name' required/>
          ...
          <button className='w-full flex justify-center item-center gap-2 bg-gradient-to-r from-[#4504f8] to-[rgb(5,198,247)] text-black px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
     ...
     <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 '>
          <div className='flex item-center gap-3'>
            <Scissors className='w-5 h-5 text-[#f80994]'/>
